...
def scatter_plot(array_dict):
    ...
    for i,pdb in enumerate(list(pdb_dict.keys())):
        ...
        axs.annotate(pdb, (i+0.25, array_dict[pdb]),fontsize=22)

    plt.xlabel('CB1 Experimental structures ',**hfont,fontsize=24)
    plt.ylabel('Volume ($\AA^3$)',**hfont,fontsize=24)
    ...
    plt.tight_layout()
    plt.savefig('Figure_1C',transparent=False,dpi =500)
...
if __name__=='__main__':
    color_dict = {'active':rgb_to_hex(int(255*0.02), int(255*0.38), int(255*0.67)),'inactive':'orange'} #colors to represent active and inactive structures 
    ...
    for pdb in list(pdb_dict.keys()):
        filename = './CB1_' + pdb_dict[pdb][0] + '_' + pdb_dict[pdb][1] + '_' + pdb + '_output.txt'  #file containing the volume output based on POVME calculations
