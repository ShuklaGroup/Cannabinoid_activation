...
def load_feature(feature):
    ...
    for i in range(6):
        for j in range(20):
            filename =  '../numpy/CB1_macrostate_' + str(i) + '_bt_' + str(j) + '_activation.npy'
            np.load('../Activation_github_code/Bootstrap_files/CB1_macrostate_' + str(i) + '_bt_' + str(j) + '_Feature.npy',np.load(filename)[:,[10,3,26,17,16]])
...
def bar_plot(macrostate_matrics,macrostate,c):
    ...
    plt.bar([i*2 for i in range(5)], activation_mean, yerr=activation_std, align='center', color=c, alpha=0.7, ecolor='black', capsize=10)
