...
def dataloader(data,key):
    if key in ['In6_chi2','TG_angle_z']:
        return_data = data[:,CB1_dica[key]]*180/np.pi
    else:
        return_data = data[:,CB1_dica[key]]*10
...
def bt_condition_probability(feature,bt_number,state):
    totdist = []
    totdist = pickle.load(open('../Bootstrap_files/CB1_bt_80_' + str(bt_number) + '_feature.pkl','rb'))
    msm = pickle.load(open('../Bootstrap_files/CB1_bt_80_' + str(bt_number) + '_msm.pkl','rb'))
    ...
    if feature in ['TG_diff_z','In57_distance','Ex16_distance']:
...
if __name__=='__main__':
    ...
    condition = 'Ex16_distance'
    features = ['Helix2M_Nloop','TG_diff_z','In36_distance','In57_distance']
    thresolds = {'Helix2M_Nloop':23,'Ex16_distance':25,'TG_diff_z':-1,'In57_distance':12,'In36_distance':14}
    bt_numbers = 200

    CB1_dica = {'Helix2M_Nloop':0,'Ex16_distance':1,'TG_diff_z':2,'In36_distance':3,'In57_distance':4}
    ...
    pickle.dump(fraction_condition,open('./CB1_'+condition+'.pkl','wb')) 
